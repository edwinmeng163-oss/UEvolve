...
namespace UnrealMcp
{
	...
	namespace
	{
		...
		FUnrealMcpExecutionResult ExecuteGeneratedBpAddInputAxisEventNodeTool(const FString& ToolName, const FJsonObject& Arguments)
		{
			FString BlueprintPath;
			FString GraphName;
			FString AxisName;
			const bool bHasBlueprintPath = Arguments.TryGetStringField(TEXT("blueprintPath"), BlueprintPath);
			const bool bHasAxisName = Arguments.TryGetStringField(TEXT("axisName"), AxisName);
			Arguments.TryGetStringField(TEXT("graphName"), GraphName);
			if (GraphName.IsEmpty())
			{
				GraphName = TEXT("EventGraph");
			}
			const bool bDryRun = Arguments.HasTypedField<EJson::Boolean>(TEXT("dryRun")) && Arguments.GetBoolField(TEXT("dryRun"));

			auto MakeError = [](const FString& Message)
			{
			...
			if (!bHasBlueprintPath || BlueprintPath.TrimStartAndEnd().IsEmpty())
			{
				return MakeError(TEXT("blueprintPath is required."));
			}
			if (!bHasAxisName || AxisName.TrimStartAndEnd().IsEmpty())
			{
				return MakeError(TEXT("axisName is required."));
			}
			...
			if (!ObjectPath.Contains(TEXT(".")))
			{
				FString PackagePath;
				FString AssetName;
				if (ObjectPath.Split(TEXT("/"), &PackagePath, &AssetName, ESearchCase::CaseSensitive, ESearchDir::FromEnd) && !AssetName.IsEmpty())
				{
			...
			if (!Blueprint)
			{
				return MakeError(FString::Printf(TEXT("Unable to load Blueprint asset: %s"), *BlueprintPath));
			}
			...
			if (!TargetGraph)
			{
				return MakeError(FString::Printf(TEXT("Blueprint graph not found: %s"), *GraphName));
			}
			...
			TSharedPtr<FJsonObject> StructuredContent = MakeShared<FJsonObject>();
			StructuredContent->SetStringField(TEXT("action"), TEXT("bp_add_input_axis_event_node"));
			StructuredContent->SetStringField(TEXT("toolName"), ToolName);
			StructuredContent->SetStringField(TEXT("blueprint"), Blueprint->GetPathName());
			StructuredContent->SetStringField(TEXT("graph"), TargetGraph->GetName());
			StructuredContent->SetStringField(TEXT("axisName"), AxisName);
			StructuredContent->SetBoolField(TEXT("dryRun"), bDryRun);
			StructuredContent->SetNumberField(TEXT("x"), X);
			StructuredContent->SetNumberField(TEXT("y"), Y);

			if (bDryRun)
			{
				FUnrealMcpExecutionResult Result;
				Result.Text = FString::Printf(TEXT("Dry run: would add InputAxisEvent '%s' to %s:%s."), *AxisName, *BlueprintPath, *GraphName);
				Result.StructuredContent = StructuredContent;
				Result.bIsError = false;
				return Result;
			}

			UClass* NodeClass = LoadObject<UClass>(nullptr, TEXT("/Script/BlueprintGraph.K2Node_InputAxisEvent"));
			if (!NodeClass)
			{
				return MakeError(TEXT("Unable to load /Script/BlueprintGraph.K2Node_InputAxisEvent."));
			}
			...
			if (!Node)
			{
				return MakeError(TEXT("Failed to allocate K2Node_InputAxisEvent."));
			}
			...
			auto ImportPropertyText = [&Warnings](UObject* Object, const FName PropertyName, const FString& TextValue) -> bool
			{
				if (!Object)
				{
					return false;
				...
				if (!Property)
				{
					Warnings.Add(FString::Printf(TEXT("Property not found on node: %s"), *PropertyName.ToString()));
					return false;
				}
				void* ValuePtr = Property->ContainerPtrToValuePtr<void>(Object);
				const TCHAR* ImportResult = Property->ImportText_Direct(*TextValue, ValuePtr, Object, PPF_None);
				if (!ImportResult)
				{
					Warnings.Add(FString::Printf(TEXT("Failed to import property %s=%s"), *PropertyName.ToString(), *TextValue));
					return false;
				}
				return true;
			};

			ImportPropertyText(Node, TEXT("InputAxisName"), AxisName);
			if (Arguments.HasTypedField<EJson::Boolean>(TEXT("consumeInput")))
			{
				ImportPropertyText(Node, TEXT("bConsumeInput"), Arguments.GetBoolField(TEXT("consumeInput")) ? TEXT("True") : TEXT("False"));
			}
			if (Arguments.HasTypedField<EJson::Boolean>(TEXT("executeWhenPaused")))
			{
				ImportPropertyText(Node, TEXT("bExecuteWhenPaused"), Arguments.GetBoolField(TEXT("executeWhenPaused")) ? TEXT("True") : TEXT("False"));
			}
			if (Arguments.HasTypedField<EJson::Boolean>(TEXT("overrideParentBinding")))
			{
				ImportPropertyText(Node, TEXT("bOverrideParentBinding"), Arguments.GetBoolField(TEXT("overrideParentBinding")) ? TEXT("True") : TEXT("False"));
			}
			...
			for (UEdGraphPin* Pin : Node->Pins)
			{
				...
				TSharedPtr<FJsonObject> PinObject = MakeShared<FJsonObject>();
				PinObject->SetStringField(TEXT("name"), Pin->PinName.ToString());
				PinObject->SetStringField(TEXT("displayName"), Pin->PinFriendlyName.IsEmpty() ? Pin->PinName.ToString() : Pin->PinFriendlyName.ToString());
			...
			StructuredContent->SetStringField(TEXT("nodeGuid"), Node->NodeGuid.ToString(EGuidFormats::DigitsWithHyphens));
			StructuredContent->SetStringField(TEXT("nodeClass"), Node->GetClass()->GetPathName());
			...
			StructuredContent->SetArrayField(TEXT("warnings"), WarningValues);

			FUnrealMcpExecutionResult Result;
			Result.Text = FString::Printf(TEXT("Added InputAxisEvent '%s' to Blueprint %s."), *AxisName, *BlueprintPath);
			Result.StructuredContent = StructuredContent;
			Result.bIsError = false;
			return Result;
		}
	...
	bool TryExecuteBlueprintTool(const FString& ToolName, const FJsonObject& Arguments, FUnrealMcpExecutionResult& OutResult)
	{
		...
		{
			OutResult = ExecuteGeneratedBpAddInputAxisEventNodeTool(ToolName, Arguments);
			return true;
		}
