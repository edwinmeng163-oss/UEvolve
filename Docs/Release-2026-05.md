...
Codex transport=unix endpoint=/var/.../codex.sock
...
```text
UEvolve Codex Bridge listening at ws://127.0.0.1:8766/uevolve
Codex transport=unix endpoint=/var/.../codex.sock     # POSIX
# or:
Codex transport=ws   endpoint=ws://127.0.0.1:<port>   # Windows or override
```
...
Codex transport=unix endpoint=/var/.../codex.sock     # POSIX
# あるいは：
Codex transport=ws   endpoint=ws://127.0.0.1:<port>   # Windows またはオーバーライド時
```
...
| Bridge `Failed to connect` | Bridge daemon 未起動 / Daemon not running | `bun run` で起動 / Start the daemon |
