...
- `Private/Tools/Actors`: actor selection, transforms, spawning, layout, batch edits. Actor query/selection, basic write tools, batch scale/tag edits, point-light edits, static-mesh actor configuration, and actor layout tools now live in `UnrealMcpActorTools.cpp`.
