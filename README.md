...
Latest pilot release: **[v0.12.0-pilot — Mac UE 5.6 + UE 5.7 Source Pilot](https://github.com/edwinmeng163-oss/UEvolve/releases/tag/v0.12.0-pilot-mac-ue56-ue57)** (prerelease, source-only, ~362 KB). Drop the zip into your `<UserProject>/Plugins/` and let UBT compile on first editor launch. Windows packaging via `Tools/package_plugin.ps1` is verified on UE 5.7.4 / Win64.
...
最新 pilot release：**[v0.12.0-pilot — Mac UE 5.6 + UE 5.7 Source Pilot](https://github.com/edwinmeng163-oss/UEvolve/releases/tag/v0.12.0-pilot-mac-ue56-ue57)**（prerelease、source-only、约 362 KB）。把 zip 放入你的 `<UserProject>/Plugins/`，让 UBT 在首次启动编辑器时编译。Windows 打包通过 `Tools/package_plugin.ps1` 已在 UE 5.7.4 / Win64 上验证。
...
最新 pilot release: **[v0.12.0-pilot — Mac UE 5.6 + UE 5.7 Source Pilot](https://github.com/edwinmeng163-oss/UEvolve/releases/tag/v0.12.0-pilot-mac-ue56-ue57)**（prerelease、source-only、約 362 KB）。zip を `<UserProject>/Plugins/` に配置し、初回エディタ起動時に UBT でコンパイルさせてください。Windows パッケージングは `Tools/package_plugin.ps1` により UE 5.7.4 / Win64 で検証済みです。
