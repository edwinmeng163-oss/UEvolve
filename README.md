...
最新 pilot release：**[v0.12.0-pilot — Mac UE 5.6 + UE 5.7 Source Pilot](https://github.com/edwinmeng163-oss/UEvolve/releases/tag/v0.12.0-pilot-mac-ue56-ue57)**（prerelease、source-only、约 362 KB）。把 zip 放入你的 `<UserProject>/Plugins/`，让 UBT 在首次启动编辑器时编译。Windows 打包可通过 `Tools/package_plugin.ps1` 预览使用；运行时验证仍待完成。
