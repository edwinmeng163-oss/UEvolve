...
Latest pilot release: **[v0.14.0-python-track-mac — macOS UE 5.6/5.7 + v0.15 chunks 1-5](https://github.com/edwinmeng163-oss/UEvolve/releases/tag/v0.14.0-python-track-mac)** (public, source-only, ~542 KB). Extract the projectroot zip next to your `<YourProject>.uproject` (NOT under `Plugins/`); UBT compiles on first editor launch. Windows full-experience zip remains draft under the `v0.14.0-python-track` tag, pending Windows verifier PASS.
...
- 140 registered MCP tools across actors, blueprint, editor, memory, scaffold, self-extension, skills, and widget categories.
...
最新 pilot release：**[v0.14.0-python-track-mac — macOS UE 5.6/5.7 + v0.15 chunks 1-5](https://github.com/edwinmeng163-oss/UEvolve/releases/tag/v0.14.0-python-track-mac)**（公开、source-only、约 542 KB）。把 projectroot zip 解压到 `<YourProject>.uproject` 同级目录（不要解到 `Plugins/` 下），UBT 在首次启动编辑器时自动编译。Windows full-experience zip 暂为 `v0.14.0-python-track` tag 草稿，等 Windows verifier PASS。
...
- 已注册 140 个 MCP tools，覆盖 actors、blueprint、editor、memory、scaffold、self-extension、skills、widget 分类。
...
最新 pilot release: **[v0.14.0-python-track-mac — macOS UE 5.6/5.7 + v0.15 chunks 1-5](https://github.com/edwinmeng163-oss/UEvolve/releases/tag/v0.14.0-python-track-mac)**（公開、source-only、約 542 KB）。projectroot zip を `<YourProject>.uproject` と同じ階層に展開してください（`Plugins/` 配下には展開しません）。UBT が初回エディタ起動時にコンパイルします。Windows full-experience zip は `v0.14.0-python-track` タグのドラフトのまま、Windows verifier PASS 待ちです。
...
- actors、blueprint、editor、memory、scaffold、self-extension、skills、widget カテゴリにまたがる 140 個の MCP tools を登録済みです。
...
- PIE start/stop.
- Actor selection, readback inspectors, transforms, spawning, layout, and batch property edits.
...
- `unreal.mcp_build_editor`, `unreal.mcp_build_game`, `unreal.mcp_build_server`, and `unreal.mcp_build_client` run Unreal Build Tool for editor, standalone game, dedicated-server, and dedicated-client targets, capture build logs under `Saved/UnrealMcp/BuildLogs`, parse key error lines, and write build status into project memory.
- `unreal.mcp_build_packaged` runs RunUAT BuildCookRun for a cooked packaged build under `Saved/StagedBuilds/<Platform>` or a project-local archive directory.
...
- `unreal.scaffold_recipe` creates bounded high-level recipes that reduce open-ended Chat exploration. The catalog covers `first_person_ground_character`, `widget_hud`, `mcp_self_extension_pipeline`, `rts_camera`, `top_down_input`, `hud_dashboard`, and `asset_naming_audit`; each recipe returns ordered tool steps, verification gates, example tool-call JSON, and can write `chat.active_task` for continuation.
